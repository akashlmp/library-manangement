        ...
        <div class="flex flex-wrap items-center justify-between max-w-screen-xl px-4 mx-auto">
            ...
            <div class="flex items-center lg:order-2">
                @auth
                    <a href="{{ url('user/profile') }}"
                        class="text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 sm:mr-2 lg:mr-0 dark:bg-purple-600 dark:hover:bg-purple-700 focus:outline-none dark:focus:ring-purple-800">Profile</a>
            ...
            <div class="items-center justify-between hidden w-full lg:flex lg:w-auto lg:order-1" id="mobile-menu-2">
                <ul class="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
                    <li>
                        <a href="{{ url('/') }}" @class([
                            $activeLink => request()->url() == url('/'),
                            $inactiveLink => request()->url() != url('/'),
                    ...
                    @hasrole('user')
                        <li>
                            <a href="{{ route('library.index') }}" @class([
                                $activeLink => request()->url() == route('library.index'),
                                $inactiveLink => request()->url() != route('library.index'),
                            ])>Library Store</a>
                        </li>
                    @endhasrole
                    @hasrole('seller')
                        <li>
                            <a href="{{ route('library.seller.index') }}" @class([
                                $activeLink => request()->url() == route('library.seller.index'),
                                $inactiveLink => request()->url() != route('library.seller.index'),
                            ])>Your Catelog</a>
                        </li>
                    @endhasrole
                    <li>
